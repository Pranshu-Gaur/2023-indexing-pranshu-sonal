...
class LSHTester:
    ...
    def run(self,name,metric,hash_family,k_vals,L_vals):
        ...
        print(name)
        print ('L\tk\tacc\ttouch\tbuild_time\tsearch_time')
        for k in k_vals:        # concatenating more hash functions increases selectivity
            ...
            for L in L_vals:    # using more hash tables increases recall
                ...
                print ("{0}\t{1}\t{2}\t{3}\t{4:.3f}\t{5:.3f}".format(L,k,float(correct)/100,float(lsh.get_avg_touched()),float(e1+e2-s1-s2),float(e3-s3)/lsh.num_queries))
...
def LSH(points, queries):
    ...
    tester = LSHTester(points = points, queries = queries, num_neighbours = 10)
